...
import com.smartapps4u.ema.compatible.ScaleGestureDetector;
...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			paint.setColor(Color.DKGRAY);
			canvas.drawRoundRect(new RectF(graphwidth-250, 20, graphwidth-90, 70), 8, 8, paint);
			paint.setColor(Color.YELLOW);
			//canvas.drawRect(new RectF(graphwidth-240, 40,graphwidth-230 , 50), paint);
			canvas.drawText("Scale", graphwidth-150,30 , paint);
			paint.setColor(Color.GREEN);
			canvas.drawText(scaleX, graphwidth-100,45 , paint);
			canvas.drawText(scaleY, graphwidth-100,60 , paint);
			/**
			 * Draw Scale Labels
			 * 
			 */if(scaleX!=null&&scaleY!=null)
				{
					...
					paint.setColor(Color.GREEN);
					canvas.drawText(scaleX, graphwidth-100,45 , paint);
					canvas.drawText(scaleY+String.valueOf(scaleunitY)+" kWh", graphwidth-100,60 , paint);
	...
	private class VerLabelsView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			paint.setTextAlign(Align.LEFT);
			int vers = verlabels.length - 1;
			double val=Double.parseDouble(verlabels[0])-Double.parseDouble(verlabels[1]);
			DecimalFormat df = new DecimalFormat("#.##");
			scaleunitY=Float.parseFloat(df.format(val));
			for (int i = 0; i < verlabels.length; i++) {
				float y = ((graphheight / vers) * i) + border;
				paint.setColor(Color.WHITE);
	...
	protected void drawScale(Canvas canvas,float graphwidth, float graphheight)
	{   float lLeft,lTop, lRight, lBottom;
	    lLeft = graphwidth-130;
	    lTop=graphheight-GraphViewConfig.BORDER-20;
	    lRight = lLeft+120;
		lBottom = lTop+45;
		
		canvas.drawRoundRect(new RectF(lLeft, lTop, lRight, lBottom), 8, 8, paint);
		/*canvas.drawText(scaleX, graphwidth-40, 40,paint);
		canvas.drawText(scaleY, graphwidth-40, 140,paint);*/
		paint.setColor(Color.YELLOW);
		canvas.drawRect(new RectF(lLeft+5, lTop+25, lLeft+20, lTop+20), paint);
		paint.setColor(Color.WHITE);
		paint.setTextAlign(Align.LEFT);
		canvas.drawText(scaleX, lLeft+25, lTop+35, paint);
